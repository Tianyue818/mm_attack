...
def main():
    ...
    parser.add_argument("--model", type=str, default='instructblip-vicuna-7b')
    args = parser.parse_args()

    torch.set_default_device('cuda')
    test_image_path = './tests/test_vqa.jpg'
    model_name = args.model
    data_path = os.path.join(root, 'experiments_VQA')
    ...
    prompt_path = './dataset/VQA/coco_captions_100.txt'
    ...
    print('Adversarial Attack of VQA')
    ...
    for ep in range(args.begin, args.end, bs):
        ...
        for i, t in enumerate(text):
            print(f'------------ {i} ---------------')
            print(t)
        print(f'Saving adversarial image to "adv_vqa_{ep}.png".')
        for i in range(bs):
            adv_image_path = os.path.join(save_path, f'adv_vqa_{i+ep}.png')
